  ...
  const fetchComments = async () => {
    const { result, error } = await commentStore.$actions.getAll();
  ...
  <template>
    ...
    <div class="tw-bg-[color:var(--theme-background-color-shade-2)] tw-rounded-sm tw-p-3">
      <div class="tw-flex tw-flex-col sm:tw-flex-row tw-space-y-4 sm:tw-space-y-0 sm:tw-space-x-2">
        <!--Chat-->
        <article
          class="tw-flex-1 tw-rounded-sm tw-bg-[color:var(--theme-background-color-shade-3)] tw-p-3 tw-overflow-y-auto tw-max-h-[27rem]">
          <aeria-icon icon="chat-text" style="--icon-size: 1.5rem">Comentarios</aeria-icon>
          <hr class="tw-border" />
          <div v-for="comment in commentStore.items"
            class="tw-m-2 tw-bg-[color:var(--theme-background-color-shade-5)] tw-rounded-sm">
            <div class="tw-flex tw-justify-between tw-items-center tw-p-2">
              <div>{{ comment.owner?.name }}</div>
              <div>{{ formatDateTime(comment.created_at) }}</div>
            </div>
            <div class="tw-border tw-m-1"></div>
