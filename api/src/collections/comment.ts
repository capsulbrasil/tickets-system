...
export const comment = extendCommentCollection({
  ...
  functions: {
    insert: async (payload: InsertPayload<Comment>, context) => {
      ...
      const allNumbers = await context.collections.comment.model
        .aggregate<PhoneNumbers>([
          {
            $match: {
              ticket: new ObjectId(payload.what.ticket as undefined),
            },
          },
          {
            $lookup: {
          ...
          },
          {
            $project: {
              _id: 0,
              phones: {
                $setUnion: ["$comment_phone", "$ticket_phone"],
              },
            },
          },
          {
        ...
        .next();
      if (allNumbers) {
        const { error: userError, result: user } =
          await context.collections.user.functions.get({
            filters: {
        ...
        for (const phone of allNumbers.phones) {
          whatsappMessage(
            phone,
            message,
            result.images?.map((image) => image.absolute_path)
          ).catch(console.trace);
        }
      }
