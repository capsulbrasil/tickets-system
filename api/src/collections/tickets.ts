import { InsertPayload, throwIfError, insert as originalInsert, Result } from "aeria";
import { extendTicketCollection, Ticket, ticketCollection } from "../../.aeria/out/collections/ticket.mjs";
  ...
  functions: {
    insert: async (payload: InsertPayload<Ticket>, context) => {
      ...
      if (insertEither.result && context.token.authenticated === true) {
        const { title, status, description, priority, attached, owner } = insertEither.result

        const files: NonNullable<MessageCreateOptions['files']>[number][] = []

        if (attached) {
          files.push({
            attachment: Buffer.from(await (await fetch(attached.download_link)).arrayBuffer()),
            name: `attachment.jpeg`,
          })
        } else {
          console.warn(`Unable to get ticket topic inserted by ${owner?.email}`)
        }
