import { Client, DiscordAPIError, GatewayIntentBits, type Message, type GuildChannelManager, type TextChannel, type Channel } from 'discord.js'
...
const checkClient = async () => {
  ...
  if(!discord.mainServerManager.client) {
    const server = await discord.client.guilds.fetch(process.env.DISCORD_MAIN_SERVER_ID)
...
type SendMessageParameters = { channelId: string, message: Parameters<typeof TextChannel.prototype.send>[0], notFromMainServer?: boolean }

const sendMessage = async (params: SendMessageParameters): Promise<Result.Error<DiscordAPIError> | Result.Result<Message<true> | any>> => {
  await checkClient()
